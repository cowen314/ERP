...
class ERP:
    ...
    def call_erp(self, input_dir: Path, output_dir: Path, segment_id: int) -> Tuple[str, bool]:
    ...
    def process_single(self, input_dir: Path, output_dir: Path, segment_ids: List[int], segmentation_volume: Path=None) -> Tuple[str, bool]:
        ...
        for id in segment_ids:
            ...
            msg, success = self.call_erp(input_dir, segment_output_dir, id)
            if not success:
        ...
        if len(errors) > 0:
            errors_formatted = '\n\t'.join(errors)
            return f"Some failures occurred while processing: {errors_formatted}", False
